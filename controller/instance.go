...
func (instance *Instance) HandleFinalizeInstanceState(databases map[string]*Database, roles map[string]*Role) {
    ...
    for _, database := range databases {
        if database.Spec.InstanceRef.Name == instance.Name {
            log.Infof("instance '%s' in namespace '%s' can't be deleted, it is referenced by database '%s' in namespace '%s'",
                instance.Name,
                instance.Namespace,
                database.Name,
                database.Namespace,
    ...
    for _, role := range roles {
        if role.Spec.InstanceRef.Name == instance.Name {
            log.Infof("instance '%s' in namespace '%s' can't be deleted, it is referenced by role '%s' in namespace '%s'",
                instance.Name,
                instance.Namespace,
                role.Name,
                role.Namespace,
